    ...
    <div class="container">
          ...
          <div class="max-w-xs">
            ...
            <template v-if="filteredCoins.length">
              <div class="flex bg-white shadow-md p-1 rounded-md shadow-md flex-wrap">
                <span
                  v-for="(item, index) in filteredCoins.slice(0, 4)"
                  :key="index"
            ...
            <div v-if="isValid" class="text-sm text-red-600">Такой тикер уже добавлен</div>
          </div>
        </div>
        <button
          @click="add()"
      ...
      <template v-if="tickers.length">
        ...
        <div class="flex justify-between items-center">
          <div>Фильтр: <input class="border" v-model="filter" /></div>
          <div>
            <button
              v-if="page > 1"
              @click="page = page - 1"
...
export default {
  ...
  methods: {
      ...
      setInterval(async () => {
        try {
          const res = await fetch(
            `https://min-api.cryptocompare.com/data/price?fsym=${currentTicker}&tsyms=USD&api_key=e81f5fe0335ffcb84f71716d0830bfb9da60416b123bd7dedd281a7d0961116b`
          );

          const data = await res.json();

          this.tickers.find((el) => el.name === currentTicker).price =
            data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2);

          if (this.selectedTicker?.name === currentTicker) {
            this.graphs.push(data.USD);
          }
        } catch (error) {
          console.log("error get coin", error);
        }
      }, 2000);
    },
    add(hintDown) {
      if (hintDown) {
        this.ticker = hintDown;
      }
      if (this.isDoubleTicker(this.ticker)) {
        this.isValid = true;
        return;
      }
    ...
    remove(name) {
      this.tickers = this.tickers.filter((el) => el.name !== name);
    ...
    isDoubleTicker(ticker) {
      return this.tickers.some((el) => el.name === ticker.toLowerCase());
    },
  },
  computed: {
    startIndex() {
    ...
    normalizedGraph() {
      ...
      return this.graphs.map((price) => 5 + ((price - minValue) * 95) / (maxValue - minValue));
    },
    pageStateOptions() {
      return {
        filter: this.filter,
  ...
  watch: {
    ...
    ticker: function (newValue, oldValue) {
      if (!this.isDoubleTicker(newValue)) {
        this.isValid = false;
      }

      if (newValue !== oldValue && newValue !== "") {
        this.filteredCoins = this.coins.filter((el) => el.Symbol.toLowerCase().includes(newValue));
      }
    },
    filter() {
      this.page = 1;
